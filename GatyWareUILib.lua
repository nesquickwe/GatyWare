...
NOTES:
GatyWareUILIB is a custom interface suite built from the ground up, meaning scripted and designed from scratch unlike Luna. If any other UIs look like GatyWareUILIB,
It is pure coincidence (Coming back after writing this, allusive looks like GatyWareUILIB a hella ton, and i didnt even know that lib existed :sob: so yea). 
...
The nature of GatyWareUILIB is a GUI Model based library (and not drawing), meaning the interface is designed in studio as a Roblox game asset, before being published to roblox
...
It Just Happens to be the same. If you're wondering why the model's interface is called GatyWareUILIB V2, its because i was working on a previous discontinued UI library project in the past and
it was also called GatyWareUILIB. Some members of the Nebula Softworks Community should know about that, and it was discontinued due to my lack of motivation
...
Main example is the way returning for elements function. Like wtf? GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index].NestedElements[NestedIndex] = NestedElement ??!!?!?!
...
local GatyWareUILIB = {
	...
	FileSystem = {
		Folder = "GatyWareUILIB Interface Suite",
		FileExtension = ".GatyWareUILIB",
...
local website = "nebulasoftworks.xyz/GatyWareUILIB"
...
GatyWareUILIB.Themes = Themes
GatyWareUILIB.CurrentTheme = deepCopy(Themes.GatyWareUILIB)
...
local ConfigMethods = {
	...
	Load = function(Idx, Data, Path)
		if GetNestedValue(GatyWareUILIB.Window.TabSections, Idx) then
			if GetNestedValue(GatyWareUILIB.Window.TabSections, Idx) then
				for key, value in pairs(Data) do
					if table.find(ClassInterfacer[GetNestedValue(GatyWareUILIB.Window.TabSections, Idx).Class], key) then
						GetNestedValue(GatyWareUILIB.Window.TabSections, Idx):Set({[key] = value})
					end
				end
			end
		end
	...
	UpdateOld = function(oldPath, newPath)
		...
		for i = 1, #list do
			local file = list[i]
			if file:sub(-#GatyWareUILIB.FileSystem.FileExtension) == GatyWareUILIB.FileSystem.FileExtension then
				local content = readfile(file)

				local pos = file:find(GatyWareUILIB.FileSystem.FileExtension, 1, true)
				local start = pos
				...
				if char == "/" or char == "\\" then
					...
					if name ~= "options" then
						writefile(`{newPath}/{name}{GatyWareUILIB.FileSystem.FileExtension}`, content)
					end
				end
	...
	bindTheme = function(object : GuiObject, property, themeKey)
		...
		local function set()
			pcall(task.defer,function()
				if object.ClassName == "UIGradient" and typeof(GetNestedValue(GatyWareUILIB.CurrentTheme, themeKey)) == "Color3" then
					object[property] = ColorSequence.new {
						ColorSequenceKeypoint.new(0, GetNestedValue(GatyWareUILIB.CurrentTheme, themeKey)),
						ColorSequenceKeypoint.new(1, GetNestedValue(GatyWareUILIB.CurrentTheme, themeKey))
					}
					return
				end

				object[property] = GetNestedValue(GatyWareUILIB.CurrentTheme, themeKey)
			end)
		end
...
local function BlurModule(Frame : Frame)
	...
	for i,v in pairs(Lighting:GetChildren()) do
		...
		if v:IsA("DepthOfFieldEffect")
			and not string.find(v.Name, "GatyWareUILIBBlur_", nil) then
	...
	local partRoot = Camera:FindFirstChild("GatyWareUILIB Blur Elements") or Instance.new("Folder", Camera)
	partRoot.Name = "GatyWareUILIB Blur Elements"
	...
	blurObject.Name = "GatyWareUILIBBlur_" .. Frame.Name .. HttpService:GenerateGUID(false)
...
local function Hide(Interface , JustHide : boolean?, Notify : boolean?, Bind : string?)
	...
	if Notify then
		if GatyWareUILIB.Instance.MobileToggle.Visible then 
			GatyWareUILIB:Notification({
...
local function AddToolTip(InfoStr, HoverInstance)
	...
	tooltip.ZIndex = 300
	tooltip.Parent = GatyWareUILIB.Instance.Tooltips
	...
	pcall(function()
		...
		UserInputService.InputChanged:Connect(function(Input)
			if Input == DragInput and Dragging then
				debounce = true
				if GatyWareUILIB.Maximized then
...
local GatyWareUILIBUI : ScreenGui = isStudio and script.Parent:WaitForChild("GatyWareUILIB V2") or game:GetObjects("rbxassetid://" .. modelId)[1]
...
repeat
	...
	if GatyWareUILIBUI.Resources:FindFirstChild('Build') and GatyWareUILIBUI.Resources.Build.Value == GatyWareUILIB.InterfaceBuild then
		correctBuild = true
		break
	end

	toDestroy, GatyWareUILIBUI = GatyWareUILIBUI, isStudio and script.Parent:FindFirstChild('GatyWareUILIB V2') or game:GetObjects("rbxassetid://" .. modelId)[1]
...
GatyWareUILIBUI.Name = (((getgenv and getgenv().InterfaceName) or GatyWareUILIBUI.Name) or "GatyWareUILIB Interface Suite")
GatyWareUILIB.Instance = GatyWareUILIBUI
GatyWareUILIBUI.Enabled = false
if not isStudio then
	pcall(function()
		GatyWareUILIBUI.OnTopOfCoreBlur = true
	end)
end

-- Sets The Interface Into Roblox's GUI
if gethui then
	GatyWareUILIBUI.Parent = gethui()

elseif syn and syn.protect_gui then 
	syn.protect_gui(GatyWareUILIBUI)

	GatyWareUILIBUI.Parent = CoreGui

elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	GatyWareUILIBUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == GatyWareUILIBUI.Name and Interface ~= GatyWareUILIBUI then
			Hide(Interface, true)
			--task.wait()
			Interface:Destroy()
		end
...
GatyWareUILIBUI.MainWindow.Visible = false
GatyWareUILIBUI.MainWindow.AnchorPoint = Vector2.zero
GatyWareUILIBUI.MainWindow.Position = UDim2.fromOffset(
	Camera.ViewportSize.X / 2 - GatyWareUILIBUI.MainWindow.Size.X.Offset / 2,
	((Camera.ViewportSize.Y / 2 - GuiInset) - GatyWareUILIBUI.MainWindow.Size.Y.Offset / 2) - (GuiInset/2)
)
GatyWareUILIBUI:WaitForChild("Drag").Position = UDim2.new(
	.5,0,0,
	((Camera.ViewportSize.Y / 2 - GuiInset) - GatyWareUILIBUI.MainWindow.Size.Y.Offset / 2) - (GuiInset/2) + GatyWareUILIBUI.MainWindow.Size.Y.Offset + 10
)

--// SUBSECTION : Interface Variables

local mainWindow : Frame = GatyWareUILIBUI.MainWindow
local Resources = GatyWareUILIBUI.Resources
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
	GatyWareUILIBUI.Notifications.Interactable = false
end

if PlayerGui:FindFirstChild("TouchGui") then
	function check()
		if PlayerGui:FindFirstChild("TouchGui").TouchControlFrame.JumpButton.Visible then
			GatyWareUILIBUI.Notifications.Position = UDim2.new(1,-20,1,-(24 + PlayerGui:FindFirstChild("TouchGui").TouchControlFrame.JumpButton.AbsoluteSize.Y))
		else
	GatyWareUILIBUI.Notifications.Position = UDim2.new(1,-20,1,-20)
		end
	end
...
function GatyWareUILIB:OnDestroy(func)
	GatyWareUILIB.DestroyFunction = func
end

-- Destroys The Interface
function GatyWareUILIB:Destroy()
	task.wait()
	GatyWareUILIBUI:Destroy()
end 
GatyWareUILIBUI.Destroying:Connect(function()
	pcall(GatyWareUILIB.DestroyFunction)
	for i,v in pairs(connections) do
		v:Disconnect()
	end
	if GatyWareUILIB.Window then
		for _, tabSection in pairs(GatyWareUILIB.Window.TabSections) do
			tabSection:Destroy()
		end
	end
	for i,v in pairs(GatyWareUILIB) do
		v = nil
	end
	if Camera:FindFirstChild("GatyWareUILIB Blur Elements") then
		for _, blur in pairs(Camera:FindFirstChild("GatyWareUILIB Blur Elements"):GetChildren()) do
			blur:Destroy()
		end
		--Camera:FindFirstChild("GatyWareUILIB Blur Elements"):Destroy()
	end
end)

function GatyWareUILIB:Notification(data)
	...
	--[[if not correctBuild and not warned then
		warned = true
		warn('GatyWareUILIB | Build Mismatch')
		warn('GatyWareUILIB may run into issues as it seems you are running an incompatible interface version ('.. (GatyWareUILIBUI.Resources:FindFirstChild("Build") and GatyWareUILIBUI.Resources:FindFirstChild("Build").Value or 'No Build') ..'). of GatyWareUILIB\n\nThis version of GatyWareUILIB is intended for interface build '..GatyWareUILIB.InterfaceBuild..'.\nTry rerunning the script. If the issue persists, join our discord for support.')
		pcall(function()
			GatyWareUILIB:Notification({
				Title = "GatyWareUILIB - Build Mistmatch",
				Content = 'GatyWareUILIB may run into issues as it seems you are running an incompatible interface version ('.. (GatyWareUILIBUI.Resources:FindFirstChild("Build") and GatyWareUILIBUI.Resources:FindFirstChild("Build").Value or 'No Build') ..'). of GatyWareUILIB\n\nThis version of GatyWareUILIB is intended for interface build '..GatyWareUILIB.InterfaceBuild..'. \nTry rerunning the script. If the issue persists, join our discord for support.',
	...
	task.spawn(function()
		...
		newNotification.Parent = GatyWareUILIBUI.Notifications
		newNotification.LayoutOrder = #GatyWareUILIBUI.Notifications:GetChildren()
		...
		newNotification.Size = UDim2.new(1, 0, 0, -GatyWareUILIBUI.Notifications:FindFirstChild("UIListLayout").Padding.Offset)
		...
		if data.Duration >= 0 then
			...
			pcall(function()
				if not GatyWareUILIB.NotificationsOpen then
					...
					Tween(newNotification, {Size = UDim2.new(1, -90, 0, -GatyWareUILIBUI.Notifications:FindFirstChild("UIListLayout").Padding.Offset)}, function()
						newNotification.Visible = false
					end, TweenInfo.new(1, Enum.EasingStyle.Exponential))
				end

				CollectionService:AddTag(newNotification, "__GatyWareUILIB_ExpiredNotification")
			end)
		end
...
-- Create the Window
function GatyWareUILIB:CreateWindow(WindowSettings)
	...
	GatyWareUILIB.FileSystem.AutoloadConfigPath = `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/`
	if WindowSettings.FileSettings.ThemesInRoot then
		GatyWareUILIB.FileSystem.AutoloadThemePath = `{GatyWareUILIB.FileSystem.Folder}/{root}/themes/`
	else
		GatyWareUILIB.FileSystem.AutoloadThemePath = `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/themes/`
	end

	GatyWareUILIB.FileSystem:BuildFolderTree( WindowSettings.FileSettings)

	GatyWareUILIB.Window = {
		Instance = mainWindow,
		TabSections = {},
		CurrentTab = nil,
		Settings = nil,
	...
	do
		...
		pcall(function()
			...
			AcrylicObject2.AddParent(GatyWareUILIBUI.MobileToggle)
			AcrylicObject2.Frame.Parent = GatyWareUILIBUI.MobileToggle
		...
		acrylicEvent.Event:Connect(function()
			...
			if mainAcrylic then
				...
				Tween(GatyWareUILIBUI.MobileToggle.Backdrop, {BackgroundTransparency = 0.5})
				Tween(GatyWareUILIBUI.MobileToggle.Backdrop.UIStroke, {Transparency = 0.5})
				...
				Tween(GatyWareUILIBUI.MobileToggle.Backdrop, {BackgroundTransparency = 0})
				Tween(GatyWareUILIBUI.MobileToggle.Backdrop.UIStroke, {Transparency = 0})
		...
		GatyWareUILIBUI.MobileToggle.Image = WindowSettings.Icon ~= nil and "rbxassetid://" .. WindowSettings.Icon or "rbxassetid://6031097229"
		...
		GatyWareUILIBUI.Drag.Position = UDim2.new(0.5, 0, 0, ((Camera.ViewportSize.Y / 2 - GuiInset) - GatyWareUILIBUI.MainWindow.Size.Y.Offset / 2) - (GuiInset/2) + mainWindow.Size.Y.Offset + 10)

		mainWindow.ModalOverlay.Visible = false
		--[[mainWindow["Loading Screen"].Version.Text = WindowSettings.LoadingSettings.Title == "GatyWareUILIB Interface Suite" and Release or "GatyWareUILIB Interface Suite " .. Release
		...
		-- Theme Binding
		do
			ThemeMethods.bindTheme(GatyWareUILIBUI.MobileToggle.Backdrop, "BackgroundColor3", "Backgrounds.Dark")
			ThemeMethods.bindTheme(GatyWareUILIBUI.MobileToggle.Backdrop.UIStroke, "Color", "Foregrounds.Dark")
			for _, shadow in pairs(GatyWareUILIBUI.MobileToggle.Backdrop.DropShadowHolder:GetChildren()) do
			...
			ThemeMethods.bindTheme(GatyWareUILIBUI.Drag.DragCosmetic, "BackgroundColor3", "Foregrounds.Light")
		...
		task.defer(function()
			if WindowSettings.LoadingEnabled then
				...
				GatyWareUILIBUI.Drag.Visible = true
				GatyWareUILIBUI.MobileToggle.Visible = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
				...
				title.Text = WindowSettings.LoadingSettings and WindowSettings.LoadingSettings.Title or "GatyWareUILIB Interface Suite"
				versionLabel.Text = title.Text == "GatyWareUILIB Interface Suite" and Release or `GatyWareUILIB UI {Release}`
				...
				subtitle.Text = WindowSettings.LoadingSettings and (WindowSettings.LoadingSettings.Subtitle or WindowSettings.LoadingSettings.Title) or "Welcome To GatyWareUILIB!"
			...
			Tween(mainWindow, {
				...
				Position = UDim2.fromOffset(
					Camera.ViewportSize.X / 2 - GatyWareUILIBUI.MainWindow.Size.X.Offset / 2 - 65/2,
					((Camera.ViewportSize.Y / 2 - GuiInset) - GatyWareUILIBUI.MainWindow.Size.Y.Offset / 2) - (GuiInset/2) - 55/2
				)
			})
			Tween(GatyWareUILIBUI.Drag, {
				Position = UDim2.new(0.5, 0, 0, ((Camera.ViewportSize.Y / 2 - GuiInset) - GatyWareUILIBUI.MainWindow.Size.Y.Offset / 2) - (GuiInset/2) + mainWindow.Size.Y.Offset + 10)
			})]]
			end
		...
		makeDraggable(mainWindow.Content.Topbar, mainWindow, GatyWareUILIBUI.Drag)
		makeDraggable(mainWindow.Sidebar, mainWindow, GatyWareUILIBUI.Drag)
		makeDraggable(GatyWareUILIBUI.MobileToggle, GatyWareUILIBUI.MobileToggle, nil)
		if GatyWareUILIBUI.Drag then makeDraggable(GatyWareUILIBUI.Drag.Interact, mainWindow, GatyWareUILIBUI.Drag, true, nil, GatyWareUILIBUI.Drag) end
	...
	function GatyWareUILIB.Window:PromptDialog(ModalSettings)
		...
		if Modal.Values.Type == 1 then
			for Key, Action in pairs(Modal.Values.Actions) do
				...
				ActionButton.MouseButton1Click:Connect(function()
					close(function()
						...
						if not Success then
							warn(`GatyWareUILIB Interface Suite - Callback Error | Dialog {Modal.Values.Name}`)
							print(Response)
			...
			for _, Action in pairs(Modal.Values.Actions) do
				...
				ActionInput.MouseEnter:Connect(function()
					Tween(ActionInput.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
				end)
				ActionInput.MouseLeave:Connect(function()
					Tween(ActionInput.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
				end)
	...
	function GatyWareUILIB.Window:CreateHomeTab(TabSettings)
		...
		if not prebuiltTabSection then
			prebuiltTabSection = GatyWareUILIB.Window:CreateTabSection()
		...
		local function Activate() -- so i dont have to rewrite shit again
			...
			Tween(Tab.Instances.Button.Icon, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Light})
			Tween(Tab.Instances.Button.Header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Light})
			Tab.Instances.Button.Icon.AccentBrighter.Enabled = true
			Tab.Instances.Button.Header.AccentBrighter.Enabled = true


			for i,v in pairs(GatyWareUILIB.Window.TabSections) do
			...
			for _, OtherTabSection in pairs(navigation:GetChildren()) do
				for _, OtherTab in pairs(OtherTabSection:GetChildren()) do
					if OtherTab.ClassName == "Frame" and OtherTab ~= Tab.Instances.Button then
						...
						Tween(OtherTab.Icon, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
						Tween(OtherTab.Header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
			...
			Tab.Active = true
			GatyWareUILIB.Window.CurrentTab = Tab
		...
		Tab.Instances.Button.MouseLeave:Connect(function()
			Tab.Hover = false
			if not Tab.Active then
				Tween(Tab.Instances.Button.Icon, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
				Tween(Tab.Instances.Button.Header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
			end
		end)
	...
	function GatyWareUILIB.Window:CreateTabSection(Name :string, Visible)
		...
		function TabSection:Set(NewName)
			...
			TabSection.Instance.Header.Text = Name
			TabSection.Instance.Name = "TAB_SECTION_"..Name
			GatyWareUILIB.Window.TabSections[Name] = TabSection
		end
		...
		function TabSection:CreateCustomTab(TabSettings, TabIndex)
			...
			if GatyWareUILIB.Window.CurrentTab == nil then
				--task.spawn(function()
				repeat
					task.wait()
				until Tab.Instances.Page.Parent == tabs
			...
			function Tab:Set(NewTabSettings)
				...
				GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Values = Tab.Values
			end
			...
			Tab.Instances.Button.Parent = GatyWareUILIB.Window.TabSections[Name].Instance
			GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex] = Tab
			return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex]
		end
		...
		function TabSection:CreateTab(TabSettings, TabIndex)
			...
			function Tab:CreateGroupbox(GroupboxSettings, GroupIndex)
				...
				function Groupbox:Set(NewGroupboxSettings)
					...
					GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Values = NewGroupboxSettings
				end
				...
				function Groupbox:CreatePrimaryButton(ElementSettings) -- these will be merged in the next update where we allow style changing.
					...
					function Element:Set(NewElementSettings)
						...
						GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name] = ElementSettings
						...
						Element.Instance["PART_Backdrop"].Header.Icon.Image = ElementSettings.Icon ~= nil and "rbxassetid://" .. Element.Values.Icon or ""
						GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name].Values = ElementSettings
					end
					...
					Element.Instance.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							warn("GatyWareUILIB Interface Suite | "..ElementSettings.Name.." Callback Error")
					...
					GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name] = Element
					return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name]
				end
					...
					UserInputService.InputBegan:Connect(function(input, processed)
						...
						if CheckingForKey then
							...
							if input.UserInputType == Enum.UserInputType.Keyboard then
								if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode[GatyWareUILIB.WindowKeybind] then
				...
				function Groupbox:CreateButton(ElementSettings, Index)
					...
					task.spawn(function()
						...
						for i, ElementInstance in pairs(Instances) do
							...
							ElementInstance.Interact.MouseButton1Click:Connect(function()
								...
								if not Success then
									...
									warn(`GatyWareUILIB Interface Suite - Callback Error | {Element.Values.Name} ({Index})`)
									print(Response)
									if WindowSettings.NotifyOnCallbackError then
										GatyWareUILIB:Notification({
						...
						function Element:Set(NewElementSettings , NewIndex)
							...
							GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index].Values = Element.Values
					...
					GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index] = Element
					return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index]
				end
				...
				function Groupbox:CreateToggle(ElementSettings, Index)
					...
					task.spawn(function()
						...
						local tooltips = {}
						local knobcolor = GatyWareUILIB.CurrentTheme.Foregrounds.Medium
						...
						local function Set(bool)
							...
							if bool then
								...
								Tween(Instances.Style2.Switch.Knob, {Position = UDim2.new(0,20,.5,0), BackgroundColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Active, BackgroundTransparency = 0})
						...
						for i, ElementInstance in pairs(Instances) do
							...
							elseif ElementInstance.Switch then
								...
								ElementInstance.Switch.MouseEnter:Connect(function()
									knobcolor = GatyWareUILIB.CurrentTheme.Foregrounds.MediumHover
								...
								ElementInstance.Switch.MouseLeave:Connect(function()
									knobcolor = GatyWareUILIB.CurrentTheme.Foregrounds.Medium
									if not Element.Values.CurrentValue then
										Tween(ElementInstance.Switch, {BackgroundColor3 = knobcolor})
										Tween(ElementInstance.Switch.Knob, {BackgroundColor3 = knobcolor})
										Tween(ElementInstance.Switch.UIStroke, {Color = knobcolor})
						...
						function Element:Set(NewElementSettings , NewIndex)
							...
							GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index].Values = Element.Values
					...
					GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index] = Element
					return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index]
				end
				...
				function Groupbox:CreateDivider()
					...
					GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["Divider_" .. Divider.ID] = Divider
					return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["Divider_" .. Divider.ID]
				end
				...
				function Groupbox:CreateInput(ElementSettings, Index)
					...
					task.spawn(function()
						...
						Element.Instance.MouseEnter:Connect(function()
							Tween(Element.Instance.PART_Backdrop.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
						end)
						Element.Instance.MouseLeave:Connect(function()
							Tween(Element.Instance.PART_Backdrop.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
						end)
				...
				function Groupbox:CreateLabel(ElementSettings, Index)
					...
					function Element:AddBind(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							task.delay(.2, function()
								NestedElement.Instance.Text = NestedElement.Values.CurrentValue == "No Bind" and "<font color=\"rgb(" .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.R*255 + 0.5)).."," .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.G*255 + 0.5))..",".. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.B*255 + 0.5)) .. ")\">No Bind</font>" or NestedElement.Values.CurrentValue
							end)
							...
							NestedElement.Instance.MouseEnter:Connect(function()
								Tween(NestedElement.Instance.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
							end)
							NestedElement.Instance.MouseLeave:Connect(function()
								Tween(NestedElement.Instance.UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
							end)
							...
							NestedElement.Instance.FocusLost:Connect(function(enter)
								if not enter then
									...
									if String.IsEmptyOrNull(NestedElement.Instance.Text) then
										...
										NestedElement.Instance.Text = "<font color=\"rgb(" .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.R*255 + 0.5)).."," .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.G*255 + 0.5))..",".. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.B*255 + 0.5)) .. ")\">No Bind</font>"
									end
								end
							end)
								...
								if CheckingForKey then
									...
									if NestedElement.Values.WindowSetting then
										...
										if input.KeyCode ~= Enum.KeyCode.Unknown then
											...
											local Success,Response = pcall(function()
												...
												GatyWareUILIB.WindowKeybind = tostring(NewKeyNoEnum)
											end)

											if not Success then
												Parent.Instance.Header.Text = "Callback Error"
												warn(`GatyWareUILIB Interface Suite - Callback Error | {Element.Values.Name} ({Index} {NestedIndex})`)
												print(Response)
												if WindowSettings.NotifyOnCallbackError then
													GatyWareUILIB:Notification({
														Title = Element.Values.Name.." Callback Error",
											...
											GatyWareUILIB.WindowKeybind = nil
										end
									...
									elseif input.UserInputType == Enum.UserInputType.Keyboard then
										if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode[GatyWareUILIB.WindowKeybind] then
											...
											NestedElement.Instance:ReleaseFocus()
										elseif input.KeyCode == Enum.KeyCode[GatyWareUILIB.WindowKeybind] then
											NestedElement.Instance.Text = NestedElement.Values.CurrentValue == "No Bind" and "<font color=\"rgb(" .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.R*255 + 0.5)).."," .. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.G*255 + 0.5))..",".. tostring(math.floor(GatyWareUILIB.CurrentTheme.Foregrounds.Medium.B*255 + 0.5)) .. ")\">No Bind</font>" or NestedElement.Values.CurrentValue
											NestedElement.Instance:ReleaseFocus()
										end
									else
							...
							local Success,Response = pcall(function()
								NestedElement.Values.OnChangedCallback(NestedElement.Values.CurrentValue)
								if NestedElement.Values.WindowSetting then GatyWareUILIB.WindowKeybind = tostring(NestedElement.Values.CurrentValue) end
							end)
							...
							function NestedElement:Set(NewNestedSettings, NewNestedIndex)
								...
								GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex].Values = NestedElement.Values
							end
						end)

						GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex] = NestedElement
						return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex]
					end
					...
					function Element:AddColorPicker(NestedSettings, NestedIndex, Parent, ParentIndex) 
						...
						task.spawn(function()
							...
							NestedElement.Instances[2].Parent = GatyWareUILIBUI.PopupOverlay
							...
							for _, TabButton in pairs(NestedElement.Instances[2].TabSelector:GetChildren()) do
								...
								TabButton.MouseButton1Click:Connect(function()
									for _, OtherTabButton in pairs(NestedElement.Instances[2].TabSelector:GetChildren()) do
										...
										Tween(OtherTabButton, {BackgroundTransparency = 1, TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
					...
					function Element:AddDropdown(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							local function hover()
								Tween(NestedElement.Instances[1].UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
								Tween(NestedElement.Instances[2].UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
								hover = true
							end
							local function leave()
								Tween(NestedElement.Instances[1].UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
								Tween(NestedElement.Instances[2].UIStroke, {Color = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
								hover = false
							end
							...
							local function Activate(option)
								pcall(function()
									...
									Tween(option.header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Light})
							...
							local function Deactivate(option)
								pcall(function()
									Tween(option, {BackgroundTransparency = 1})
									Tween(option.header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
								...
								for _, option in ipairs(NestedElement.Values.Options) do
									...
									optioninstance.MouseEnter:Connect(function()
										if optioninstance:GetAttribute("Active") == false then
											...
											Tween(optioninstance.header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Light})
										end
									end)
									optioninstance.MouseLeave:Connect(function()
										if optioninstance:GetAttribute("Active") == false then
											Tween(optioninstance, {BackgroundTransparency = 1})
											Tween(optioninstance.header, {TextColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Medium})
										end
									end)
								end
							end
				...
				function Groupbox:CreateParagraph(ElementSettings, Index)
					...
					task.spawn(function()
						...
						function Element:Set(NewElementSettings , NewIndex)
							...
							GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[NewIndex].Values = ElementSettings
						end
				...
				GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex] = Groupbox
				return GatyWareUILIB.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex]
			end
			...
			function Tab:BuildThemeGroupbox(Column, Style, ButtonsCentered)
				...
				local themesPath = WindowSettings.FileSettings.ThemesInRoot and `{GatyWareUILIB.FileSystem.Folder}/{root}/themes` or `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/themes`
				...
				local themesArray = {
					"GatyWareUILIB", "Hollywood Dark", "Hollywood Light", "Orca", "Glacier", "Pacific", "Neo", "Neo (Dark)", "Crimson", "Nebula", "Evergreen", "Luna", "OperaGX", "BBot", "Ubuntu", "Tokyo Night", "Hollywood Fluent"
				}
				local customThemes = not isStudio and GatyWareUILIB.FileSystem:RefreshConfigList(themesPath) or {}
				...
				do
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Dark,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Backgrounds.Dark = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Dark }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Medium,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Backgrounds.Medium = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Medium }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Light,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Backgrounds.Light = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Light }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Groupbox,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Backgrounds.Groupbox = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Groupbox }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Highlight,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Backgrounds.Highlight = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Backgrounds.Highlight }, nil, true)
							end
						end)
					end
				end
				...
				do
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Dark,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.Dark = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Dark }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Medium,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.Medium = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Medium }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Light,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.Light = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Light }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Active,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.Active = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.Active }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fga:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fga:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.MediumHover,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Foregrounds.MediumHover = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Foregrounds.MediumHover }, nil, true)
							end
						end)
					end
				end
				...
				do
					...
					local cp = colorpickers.divider:AddColorPicker({
						IgnoreConfig = true,
						CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.Divider,
						Callback = function(c)
							debounce = true
							GatyWareUILIB.CurrentTheme.Miscellaneous.Divider = c
							themeEvent:Fire()
							task.wait(6/60)
							debounce = false
						end,
					...
					themeEvent.Event:Connect(function()
						if not debounce then
							cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.Divider }, nil, true)
						end
					end)
				end
				...
				do
					do
						...
						local cp = colorpickers.shadows:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.Shadow,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Miscellaneous.Shadow = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.Shadow }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.shadows:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.LighterShadow,
							Callback = function(c)
								debounce = true
								GatyWareUILIB.CurrentTheme.Miscellaneous.LighterShadow = c
								themeEvent:Fire()
								task.wait(6/60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Miscellaneous.LighterShadow }, nil, true)
							end
						end)
					end
				end
					...
					do
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[1].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Main = ColorSequence.new {
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[1].Value }, nil, true)
								end
							end)
						end
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[2].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Main = ColorSequence.new {
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[2].Value }, nil, true)
								end
							end)
						end
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[3].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Main = ColorSequence.new {
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Main.Keypoints[3].Value }, nil, true)
								end
							end)
						end
					end
					...
					do
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[1].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Brighter = ColorSequence.new {
										ColorSequenceKeypoint.new(keypoints[1].Time, c),
										keypoints[2],
										keypoints[3],
									}
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[1].Value }, nil, true)
								end
							end)
						end
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[2].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Brighter = ColorSequence.new {
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[2].Value }, nil, true)
								end
							end)
						end
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[3].Value,
								Callback = function(c)
									debounce = true
									local keypoints = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints
									GatyWareUILIB.CurrentTheme.Accents.Brighter = ColorSequence.new {
							...
							themeEvent.Event:Connect(function()
								if not debounce then
									cp:Set({ CurrentValue = GatyWareUILIB.CurrentTheme.Accents.Brighter.Keypoints[3].Value }, nil, true)
								end
							end)
						end
					end
				...
				instance:CreateButton({
					...
					Callback = function()	
						...
						if isfile(`{themesPath}/{newName.CurrentValue}{GatyWareUILIB.FileSystem.FileExtension}`) or themesArray[newName.CurrentValue] then
							GatyWareUILIB:Notification({
								Title = "Theme Exists",
								Icon = 129398364168201,
						...
						local success, returned = pcall(function()
							...
							local fullPath = `{themesPath}/{newName.CurrentValue}{GatyWareUILIB.FileSystem.FileExtension}`

							local success, encoded = ThemeMethods.encodeTheme(GatyWareUILIB.CurrentTheme)
							if not success then
				...
				themeEvent.Event:Connect(function()
					for key, theme in pairs(Themes) do
						if theme == GatyWareUILIB.CurrentTheme then
				...
				instance:CreateButton({
					...
					Callback = function()
						if Themes[newThemeToApply] ~= nil then
							GatyWareUILIB:SetTheme(Themes[newThemeToApply])
						else
							GatyWareUILIB:SetTheme(ThemeMethods.decodeTheme(readfile(`{themesPath}/{newThemeToApply}{GatyWareUILIB.FileSystem.FileExtension}`)))
						end
					end,
				}, "applytheme")
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						local success, returned = pcall(function()
							...
							local fullPath = `{themesPath}/{newThemeToApply}{GatyWareUILIB.FileSystem.FileExtension}`

							local success, encoded = pcall(HttpService.JSONEncode, HttpService, GatyWareUILIB.CurrentTheme)
							if not success then
								return false, "Unable to encode into JSON data"
							end
				...
				local loadlabel = instance:CreateParagraph({
					...
					Content = not isStudio and (isfile(`{themesPath}/autoload.txt`) and readfile(`{themesPath}/autoload.txt`)) or "GatyWareUILIB",
				}, "autoloadlabel")
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if isfile(`{themesPath}/{newThemeToApply}{GatyWareUILIB.FileSystem.FileExtension}`) then
							delfile(`{themesPath}/{newThemeToApply}{GatyWareUILIB.FileSystem.FileExtension}`)
						end
			...
			function Tab:BuildConfigGroupbox(Column, Style, ButtonsCentered)
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if isfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/{inputPath.CurrentValue}{GatyWareUILIB.FileSystem.FileExtension}`) then
							GatyWareUILIB:Notification({
						...
						local success, returned = GatyWareUILIB.FileSystem:SaveConfig(inputPath.CurrentValue, `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/`)
						if not success then
							GatyWareUILIB:Notification({
								Title = "Configuration Error",
								Icon = 6031071057,
						...
						instance.Elements["__prebuiltConfigSelector_lbl"].NestedElements["__prebuiltConfigSelector_lbl"]:Set({ Options = GatyWareUILIB.FileSystem:RefreshConfigList(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs`) })
					end,
					Style = 1,
				...
				local configSelection = instance:CreateLabel({
					...
					Options = GatyWareUILIB.FileSystem:RefreshConfigList(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs`),
					CurrentOption = nil,
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						local success, returned = GatyWareUILIB.FileSystem:LoadConfig(selectedConfig, `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/`)
						if not success then
							GatyWareUILIB:Notification({
								Title = "Configuration Error",
								Icon = 6031071057,
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						local success, returned = GatyWareUILIB.FileSystem:SaveConfig(selectedConfig, `{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/`)
						if not success then
							GatyWareUILIB:Notification({
								Title = "Configuration Error",
								Icon = 6031071057,
				...
				local loadlabel = instance:CreateParagraph({
					...
					Content = isfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) and readfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) or "None",
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						pcall(function()
							writefile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/autoload.txt`, name)
						end)
						loadlabel:Set({ Content = name })
				...
				instance:CreateButton({
					...
					Callback = function()
						if isfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) then delfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) end
						loadlabel:Set({ Content = "None" })
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if isfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/{selectedConfig}{GatyWareUILIB.FileSystem.FileExtension}`) then
							delfile(`{GatyWareUILIB.FileSystem.Folder}/{folderpath}/configs/{selectedConfig}{GatyWareUILIB.FileSystem.FileExtension}`)
						end
		...
		GatyWareUILIB.Window.TabSections[Name] = TabSection
		return GatyWareUILIB.Window.TabSections[Name]
	...
	do
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseEnter"]:Connect(function()
			Tween(mainWindow.Content.Topbar.NotificationCenterIcon, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
		end)
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseLeave"]:Connect(function()
			Tween(mainWindow.Content.Topbar.NotificationCenterIcon, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
		end)
		...
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseButton1Click"]:Connect(function()
			if not notifdebounce then
				notifdebounce = true
				if GatyWareUILIB.NotificationsOpen then
					for i,newNotification in pairs(CollectionService:GetTagged("__GatyWareUILIB_ExpiredNotification")) do
				...
				GatyWareUILIB.NotificationsOpen = not GatyWareUILIB.NotificationsOpen
				task.wait(1)
				notifdebounce = false
			end
		end)
		...
		mainWindow.Content.Topbar.Search["MouseEnter"]:Connect(function()
			Tween(mainWindow.Content.Topbar.Search, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.DarkHover})
		end)
		mainWindow.Content.Topbar.Search["MouseLeave"]:Connect(function()
			Tween(mainWindow.Content.Topbar.Search, {ImageColor3 = GatyWareUILIB.CurrentTheme.Foregrounds.Dark})
		end)
		...
		mainWindow.Content.Topbar.Controls.Close["MouseButton1Click"]:Connect(function()
			GatyWareUILIB.Window:PromptDialog({
				Name = "Are you sure?",
		...
		mainWindow.Content.Topbar.Controls.Minimize["MouseButton1Click"]:Connect(function()
			if not debounce then
				debounce = true
				Hide(mainWindow, false, true, GatyWareUILIB.WindowKeybind)
				Hide(GatyWareUILIBUI.Drag, false, false, GatyWareUILIB.WindowKeybind)
		...
		GatyWareUILIBUI.MobileToggle.MouseButton1Click:Connect(function()
			if GatyWareUILIB.Minimized == true then
				if not debounce then
					debounce = true
					Unhide(mainWindow)
					Unhide(GatyWareUILIBUI.Drag)
			...
			elseif GatyWareUILIB.Minimized == false then
				if not debounce then
					debounce = true
					Hide(mainWindow, false, true, GatyWareUILIB.WindowKeybind)
					Hide(GatyWareUILIBUI.Drag, false, false, GatyWareUILIB.WindowKeybind)
		...
		connections["__windowKeybindHidingBindConnection"] = UserInputService.InputBegan:Connect(function(input, gpe)
			...
			if input.KeyCode == Enum.KeyCode[GatyWareUILIB.WindowKeybind] then
				if GatyWareUILIB.Minimized == true then
					if not debounce then
						debounce = true
						Unhide(mainWindow)
	...
	-- Return the window
	return GatyWareUILIB.Window

end

--// SECTION : Config System

function GatyWareUILIB.FileSystem:BuildFolderTree(FileSettings)
	...
	if FileSettings.RootFolder ~= nil then
		...
		if FileSettings.ThemesInRoot then
			paths = {
				GatyWareUILIB.FileSystem.Folder,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}`,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}/configs`,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}/themes`
			}
		else
			paths = {
				GatyWareUILIB.FileSystem.Folder,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}`,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}/configs`,
				`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.RootFolder}}/{FileSettings.ConfigFolder}/themes`
			}
		end
	else
		-- no root
		paths = {
			GatyWareUILIB.FileSystem.Folder,
			`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.ConfigFolder}`,
			`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.ConfigFolder}/configs`,
			`{GatyWareUILIB.FileSystem.Folder}/{FileSettings.ConfigFolder}/themes`
		}
	end
...
function GatyWareUILIB.FileSystem:SaveConfig(file, path)
	...
	local fullPath = `{path}{file}{GatyWareUILIB.FileSystem.FileExtension}`

	local data = {
		objects = {}
	}

	for tsecidx, tabsection in next, GatyWareUILIB.Window.TabSections do 
...
function GatyWareUILIB.FileSystem:LoadConfig(file, path)
...
function GatyWareUILIB.FileSystem:RefreshConfigList(path)
...
function GatyWareUILIB:LoadAutoloadConfig()
	if isStudio or (not isfile) then return "Config system unavailable." end

	if GatyWareUILIB.FileSystem.AutoloadConfigPath and isfile(GatyWareUILIB.FileSystem.AutoloadConfigPath .. "autoload.txt") then

		local name = readfile(GatyWareUILIB.FileSystem.AutoloadConfigPath .. "autoload.txt")

		local success, err = GatyWareUILIB.FileSystem:LoadConfig(name, GatyWareUILIB.FileSystem.AutoloadConfigPath)
		if not success then 
			GatyWareUILIB:Notification({
...
function GatyWareUILIB:SetTheme(newTheme)
	
	local themeToCopy = newTheme
	if type(themeToCopy) == "string" then
		themeToCopy = GatyWareUILIB.Themes[themeToCopy]
	end

	GatyWareUILIB.CurrentTheme = deepCopy(themeToCopy)
	themeEvent:Fire()

end

function GatyWareUILIB:LoadAutoloadTheme()
	if isStudio or (not isfile) then return "Config system unavailable." end

	if GatyWareUILIB.FileSystem.AutoloadThemePath and isfile(GatyWareUILIB.FileSystem.AutoloadThemePath .. "autoload.txt") then

		local name = readfile(GatyWareUILIB.FileSystem.AutoloadThemePath .. "autoload.txt")

		if Themes[name] then
			GatyWareUILIB:SetTheme(name)
		else
			local content = readfile(GatyWareUILIB.FileSystem.AutoloadThemePath .. name .. GatyWareUILIB.FileSystem.FileExtension)
			local success, decoded = pcall(HttpService.JSONDecode, HttpService, content)
			if not success then return false, "Unable to decode JSON data." end

			GatyWareUILIB:SetTheme(decoded)
		end
...
if isStudio and enabled then
	--GatyWareUILIB:SetTheme("Hollywood Dark")

	local win = GatyWareUILIB:CreateWindow({
		Name = "Window",
		...
		LoadingSettings = {
			Title = "GatyWareUILIB Interface Suite",
			Subtitle = "Welcome to GatyWareUILIB",
		},
		...
		KeySystem = {
			Enabled = false,
			Title = "GatyWareUILIB Key System",
	...
	--[[task.delay(4, function()
		GatyWareUILIB:SetTheme("GatyWareUILIB")
	end)]]
	...
	GatyWareUILIB:LoadAutoloadConfig()
end--]=]

--// ENDSECTION

return GatyWareUILIB
                    instance:CreateButton({
                    Name = "Set Autoload",
                    Icon = 6031071057,
                    CenterContent = ButtonsCentered,
                    Tooltip = "Sets the selected configuration to be automatically loaded on your future sessions.",
